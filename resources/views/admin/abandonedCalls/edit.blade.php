...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.abandonedCall.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.abandoned-calls.update", [$abandonedCall->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('c_status') ? 'is-invalid' : '' }}" name="c_status" id="c_status">
                    ...
                    @foreach(App\Models\AbandonedCall::C_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('c_status', $abandonedCall->c_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('c_action') ? 'is-invalid' : '' }}" name="c_action" id="c_action">
                    ...
                    @foreach(App\Models\AbandonedCall::C_ACTION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('c_action', $abandonedCall->c_action) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('c_reason') ? 'is-invalid' : '' }}" name="c_reason" id="c_reason">{{ old('c_reason', $abandonedCall->c_reason) }}</textarea>
